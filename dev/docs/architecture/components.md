...
|------|---------|---------|
| `background.cljs` | Service Worker | WebSocket management, script injection orchestration (including `maybe-inject-installer!`), icon state |
...
| `bg_inject.cljs` | Service Worker | Script injection primitives (bridge, Scittle, userscripts), installer scan helpers (`execute-in-isolated`, `scan-for-userscripts-fn`, `ensure-scittle!`, `execute-plan!`) |
...
### User-facing Builtin Userscripts
