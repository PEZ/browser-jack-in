...
(defn- attach-ws-handlers!
  "Attach event handlers to a WebSocket for a given tab."
  [ws dispatch! tab-id port]
  (set! (.-onopen ws)
        (fn []
          (log/debug "Background:WS" "Connected for tab:" tab-id)
  ...
  (set! (.-onerror ws)
        (fn [error]
          ...
          (send-to-tab tab-id {:type "ws-error"
                               :error (str "WebSocket error connecting to ws://localhost:" port "/_nrepl")})))
  (set! (.-onclose ws)
        (fn [event]
          (log/debug "Background:WS" "Closed for tab:" tab-id)
...
(defn- create-and-register-ws!
  "Create a WebSocket connection, register it, and attach handlers."
  [dispatch! tab-id port tab-info]
  (let [ws-url (str "ws://localhost:" port "/_nrepl")]
    (log/debug "Background:WS" "Connecting to:" ws-url "for tab:" tab-id)
    (try
      (let [ws (js/WebSocket. ws-url)]
        (dispatch! [[:ws/ax.register tab-id
                     (merge {:ws/socket ws :ws/port port} tab-info)]])
        (dispatch! [[:history/ax.track tab-id port]])
        (attach-ws-handlers! ws dispatch! tab-id port))
      (catch :default e
   ...
   (fn [tab]
     (let [tab-title (if js/chrome.runtime.lastError
           ...
           tab-url (when-not js/chrome.runtime.lastError
                     (.-url tab))]
       (create-and-register-ws! dispatch! tab-id port
