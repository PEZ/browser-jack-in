...
(defn handle-action [state uf-data [action & args]]
  (case action
    ...
    :banner/ax.track-bulk-name {:uf/db (update-in state [:ui/system-bulk-names (first args)] (fnil conj []) (second args))}
    :banner/ax.clear-bulk-names {:uf/db (update state :ui/system-bulk-names dissoc (first args))}
