...
(defn- ^:async phase-create-and-inspect!
  "Phase 1-2: Create a new script via panel, then inspect it from popup."
  [context ext-id]
...
(defn- ^:async phase-save-over-existing!
  "Phase 3-4: Update code without changing name (save over existing), verify count, inspect again."
  [context ext-id]
  (let [panel (js-await (create-panel-page context ext-id))
        textarea (.locator panel "#code-area")
        save-btn (.locator panel "button.btn-save")
...
(defn- ^:async phase-create-renamed-and-verify!
  "Phase 5-6: Change name in manifest (triggers Create), verify both scripts exist."
  [context ext-id]
  (let [panel (js-await (create-panel-page context ext-id))
        textarea (.locator panel "#code-area")
        save-btn (.locator panel "button.btn-save")
...
(defn- ^:async test_create_new_script_when_name_changed_save_when_unchanged []
  (let [context (js-await (launch-browser))
    ...
    (try
      (js-await (phase-create-and-inspect! context ext-id))
      (js-await (phase-save-over-existing! context ext-id))
      (js-await (phase-create-renamed-and-verify! context ext-id))
      (finally
