...
(defn- settled-rm-poll-value [check-result]
  (when (and (.-success check-result) (seq (.-values check-result)))
    (let [s (unquote-result (first (.-values check-result)))]
      (when (not= s ":pending") s))))

(defn- ^:async poll-rm-nonexistent-result! []
  (let [poll-code "(let [r @!rm-nonexistent-result] (cond (= r :pending) :pending (:rejected r) (:rejected r) :else r))"
        timeout 3000
        start (.now js/Date)]
    (loop []
      (let [result-str (settled-rm-poll-value (js-await (eval-in-browser poll-code)))]
        (cond
          result-str (-> (expect result-str)
                         (.toBe "Not deleting non-existent file: nonexistent_script_12345.cljs"))
          (> (- (.now js/Date) start) timeout) (throw (js/Error. "Timeout waiting for !rm-nonexistent-result"))
...
(defn- ^:async failed_fs_operation_rejects_promise []
  ...
  (let [cleanup-code "(def !cleanup-result (atom :pending))
                      (defn ^:async do-cleanup []
                        (try
                          (let [_ (await (epupp.fs/rm! \"nonexistent_script_12345.cljs\"))]
                            (reset! !cleanup-result :done))
                          (catch :default _
                            (reset! !cleanup-result :done))))
                      (do-cleanup)
                      :cleanup-started"]
  ...
  (let [delete-code "(def !rm-nonexistent-result (atom :pending))\n                                        (defn ^:async do-it []\n                                          (try\n                                            (let [r (await (epupp.fs/rm! \"nonexistent_script_12345.cljs\"))]\n                                              (reset! !rm-nonexistent-result {:resolved r}))\n                                            (catch :default e\n                                              (reset! !rm-nonexistent-result {:rejected (.-message e)}))))\n                                        (do-it)\n                                        :setup-done"
        res (js-await (helpers/eval-in-browser delete-code))]
    (-> (expect (.-success res)) (.toBe true))
    (js-await (poll-rm-nonexistent-result!))))
